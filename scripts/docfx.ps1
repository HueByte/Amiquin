...
<#
.SYNOPSIS
    Runs DocFX to build documentation for the Amiquin project.
...
try {
    Write-ColorOutput "📚 Amiquin DocFX Runner" $InfoColor
    ...
    switch ($Action) {
        "Clean" {
            ...
            $generatedPath = Join-Path $repoRoot "generated"
            
            if (Test-Path $generatedPath) {
                Remove-Item -Recurse -Force $generatedPath
                Write-ColorOutput "✅ Removed generated directory" $SuccessColor
            }
