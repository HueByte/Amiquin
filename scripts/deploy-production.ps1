    ...
    foreach ($service in $services) {
        ...
        do {
            ...
            if ($elapsed -ge $timeout) {
                Write-Host "❌ $($service.Name) health check timed out after $timeout seconds" -ForegroundColor Red
                docker logs --tail 20 $service.Container
                exit 1
            }
...
function Show-DeploymentSummary {
    ...
    Write-Host "  • Qdrant Vector DB: localhost:6333 (REST), localhost:6334 (gRPC)"
    Write-Host "  • Amiquin Bot: localhost:10001"
    ...
    if ($EnableBackups) {
        ...
        Write-Host "  • Database backups: Enabled (daily at 2:00 AM)"
        Write-Host "  • Configuration backups: Enabled"
